...
class VocabularyTab():
    ...
    def __init__(self, tab_control):
        ...
        english_language  = ttk.Radiobutton(language_text, text = "English", value = 'english', variable = self.language)
        english_language.grid(column = 0, row = 0, pady = 6, padx = 6)
        japanese_language = ttk.Radiobutton(language_text, text = self.japanese, value = 'japanese', variable = self.language)
        ...
        questions_text = ttk.Label(how_many_box, text = "How many questions per test?")
        questions_text.grid(column = 0, row = 0, sticky = tk.W, pady = 6, padx = 6)
        self.questions_per_test_input = ttk.Entry(how_many_box, width = self.WIDTH, textvariable = self.questions_per_test)
        ...
        self.tests_amount_input = ttk.Entry(how_many_box, width = self.WIDTH, textvariable = self.amount_of_tests)
        self.tests_amount_input.grid(column = 1, row  = 1, sticky = tk.W, pady = 6, padx = 6)

        save_directory_box = ttk.LabelFrame(self.vocab_tab)
        save_directory_box.grid(column = 0, columnspan = 2, row = 3, padx = 6, pady = 6)
        save_button = ttk.Button(save_directory_box, text = "Make Test(s)", command = self.vocab_tests)
        save_button.grid(column = 0, row = 0, padx = 6, pady= 6)

        reset_button = ttk.Button(save_directory_box, text = "Reset", command = self.reset_choices)
        reset_button.grid(column = 1, row = 0, padx = 6, pady = 6)

    def is_valid_input(self):
        """Validates the user input, returns Boolean."""
        if check.questions(self.questions_per_test.get(), self.MAX_QUESTIONS, self.MIN_QUESTIONS) \
           and check.tests(self.amount_of_tests.get(), self.MAX_TESTS, self.MIN_TESTS) \
           and check.grade(self.student_grade_level.get(), self.MAX_GRADE, self.MIN_GRADE) \
           and check.language(self.language.get()) \
           and check.from_(self.from_page.get(), self.until_page.get(), self.MIN_PAGE) \
           and check.until(self.until_page.get(), self.MAX_PAGE, self.from_page.get()):
            return True
        else:
            return False
