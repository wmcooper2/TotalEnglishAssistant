...
class SentenceTab():
    ...
    def __init__(self, tab_control):
        ...
        self.student_grade_level_input = ttk.Entry(grade_box, width = self.WIDTH, textvariable = self.student_grade_level)
        ...
        self.from_page_input = ttk.Entry(page_range_box, width = self.WIDTH, textvariable = self.from_page)
        self.from_page_input.grid(column = 1, row = 0, padx = 6, pady = 6)

        input_frame = ttk.LabelFrame(self.sentence_tab, text = "Enter any sentence")
        input_frame.grid(column = 0, row = 1, padx = 6, pady = 6)		
        self.sentence_input = ttk.Entry(input_frame, width = self.MAX_SENTENCE)
        self.sentence_input.grid(column = 0, row = 0, columnspan = 2, padx = 6, pady = 6)
        
        check_sentence_button = ttk.Button(input_frame, text ="Check", command = self.results)
        check_sentence_button.grid(column = 0, row = 1, padx = 6, pady = 6)

        reset_button = ttk.Button(input_frame, text = "Reset", command = self.reset_choices)
        ...
        self.until_page_input = ttk.Entry(page_range_box, width = self.WIDTH, textvariable = self.until_page)
    ...
    def is_valid_input(self):
        ...
        return check.sentence_input(self.sentence_input.get(), self.MAX_LENGTH)
    ...
    def results(self):
        """Shows results of the inputted sentence in the GUI, returns None."""
    ...
    def make_groups(self):
        ...
        for word in self.words:
            ...
            temp["word"] = word
            if word.is_valid:
                temp["result"] = ttk.Label(self.results_frame, text = word.english)
                temp["grade"] = ttk.Label(self.results_frame, text = word.grade())
                temp["page"] = ttk.Label(self.results_frame, text = word.page_number())
                temp["verb"] = ttk.Label(self.results_frame, text = word.base_verb())
                temp["noun"] = ttk.Label(self.results_frame, text = word.base_noun())
            else:
                temp["result"] = ttk.Label(self.results_frame, text = word.english)
    ...
    def draw_result_labels(self):
        ...
        ttk.Label(self.results_frame, text = "given word:").grid(column = 0, row = 0, padx = 6, pady = 6, sticky = tk.E)
        ttk.Label(self.results_frame, text = "grade:").grid(column = 0, row = 1, padx = 6, pady = 6, sticky = tk.E)
        ttk.Label(self.results_frame, text = "page:").grid(column = 0, row = 2, padx = 6, pady = 6, sticky = tk.E)
        ttk.Label(self.results_frame, text = "base verb:").grid(column = 0, row = 3, padx = 6, pady = 6, sticky = tk.E)
        ttk.Label(self.results_frame, text = "singular noun:").grid(column = 0, row = 4, padx = 6, pady = 6, sticky = tk.E)
    ...
    def draw_results_frame(self):
        ...
        self.results_frame = ttk.LabelFrame(self.sentence_tab, text = "Results" )
