...
class DictionaryTab():
    ...
    def __init__(self, tab_control):
        ...
        current_dictionary = ttk.Label(self.center_box, text = "Current dictionary: " + self.dictionary.default_dict_name)
        current_dictionary.grid(column = 0, row = 0, padx = 6, pady = 6, sticky = tk.W) 
        ...
        self.new_dict_btn = ttk.Button(self.new_dict_box, text = "New Dictionary", command = self.make_new_dict)
        self.new_dict_btn.grid()
        
        self.load_new_btn = ttk.Button(self.new_dict_box, text = "Load Dictionary", command = self.load_dict)
        self.load_new_btn.grid(column = 0, row = 1, padx = 6, pady = 6, sticky = tk. N)
    ...
    def draw_entry_box(self):
        ...
        request_box = ttk.LabelFrame(self.center_box, text = "Get entry information")
        ...
        request_button = ttk.Button(request_box, text= "Display Entry", command = self.draw_entry)
        request_button.grid(column = 0, row = 1, padx = 6, pady = 6)

        word_edit_box = ttk.Button(request_box, text= "Edit Entry", command = self.draw_edits)
    ...
    def reset_value_edits(self):
        """Resets the objects waiting for editing in the GUI, returns None."""
        self.value_edits = []

    def make_edit_objects(self):
        """Makes the objects to edit so they can be displayed, returns None."""
        row_counter = 0
        for key, value in self.entry.items():
    ...
    def add_key(self, key, row_counter):
        ...
        key_object.grid(column = 0, row = row_counter, padx = 6, pady = 6, sticky = tk.W)
    ...
    def add_value(self, value, row_counter):
        ...
        value_object.grid(column = 1, row = row_counter, padx = 6, pady = 6, sticky = tk.W)
    ...
    def draw_save_button(self):
        ...
        save_button = ttk.Button(self.word_results, text = "Save Changes", command = self.save_)
    ...
    def user_copy(self):
        """Makes a copy of the current dict for the user in '~/TotalEnglishAssistant/Dictionaries/'.
           Returns None."""
