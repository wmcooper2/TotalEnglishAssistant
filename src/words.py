...
class Word():
    ...
    def __init__(self, word):
        """Initializes Word() instance with default attributes, returns None."""
        self.english = word
    ...
    def base_word(self):
        """Checks if a word is a normal key in the dictionary, returns Boolean."""
        pass
    ...
    def set_verb_form_attribute_(self):#logic doesnt seem right at first glance
    ...
    def choose_different_word(self, some_list): 
        """Chooses a different word from a list without choosing the same word,
            returns None."""
        copy = some_list[:]
        ...
        for verb in self.verbs:
            for key, value in self.verb_forms.items():
                for k, v in value.items():
                    if v == self.english: #use re.match() for "am/is/are" problem
                        return key
